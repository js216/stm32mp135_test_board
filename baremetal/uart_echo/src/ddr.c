...
/* 1 MB PRBS write/read-back at the start of DDR.  Uses a 32-bit
 * xorshift LFSR seeded from a fixed constant so the same sequence is
 * regenerated on read-back.  Reports first mismatch offset, or
 * "DDR PRBS: 1 MB OK" on full match. */
void ddr_prbs_test(int argc, uint32_t arg1, uint32_t arg2, uint32_t arg3)
{
   (void)argc;
   (void)arg1;
   (void)arg2;
   ...
   const uint32_t WORDS = (1024U * 1024U) / 4U;
   volatile uint32_t *p = (volatile uint32_t *)DEF_DDR_BASE;

   uint32_t s = 0xACE1BEEFU;
   ...
   for (uint32_t i = 0; i < WORDS; i++) {
      ...
      uint32_t got = p[i];
      if (got != s) {
         my_printf("DDR PRBS: FAIL @ word %lu: got=%08lx exp=%08lx\r\n",
                   (unsigned long)i, (unsigned long)got, (unsigned long)s);
         return;
      }
   }
   my_printf("DDR PRBS: 1 MB OK\r\n");
}
