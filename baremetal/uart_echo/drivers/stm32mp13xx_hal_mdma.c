/**
  ...
  ******************************************************************************
  ...
  circular (last node connected to first one), if removing node1 (node where the
  linked list loops),the linked list remains circular and node 2 becomes the
  first one. Note that if the linked list is made circular the transfer will
 ...
 * contains the configuration information for the specified MDMA Channel.
 * @param  CallbackID:            User Callback identifier
 *                                a HAL_MDMA_CallbackIDTypeDef ENUM as
 * parameter.
 * @retval HAL status
 */
HAL_StatusTypeDef
                            ...
                            HAL_MDMA_CallbackIDTypeDef CallbackID)
{
   ...
   if (HAL_MDMA_STATE_READY == hmdma->State) {
      switch (CallbackID) {
         case HAL_MDMA_XFER_CPLT_CB_ID: hmdma->XferCpltCallback = NULL; break;
         ...
         case HAL_MDMA_XFER_ERROR_CB_ID: hmdma->XferErrorCallback = NULL; break;

         case HAL_MDMA_XFER_ABORT_CB_ID: hmdma->XferAbortCallback = NULL; break;
                               ...
                               MDMA_LinkNodeConfTypeDef *pNodeConfig)
{
   ...
   */
   ...
   /* if HW request set the HW request and the request CleraMask and ClearData
    * MaskData,  */
   if (pNodeConfig->Init.Request != MDMA_REQUEST_SW) {
      ...
      if ((uint32_t)hmdma->FirstLinkedListNodeAddress == 0U) {
         ...
         while ((counter < hmdma->LinkedListNodeCounter) &&
            ...
            if (pNode->CLAR == (uint32_t)pNewNode) {
               hal_status =
                   HAL_ERROR; /* error this node already exist in the linked
                 ...
                 pNode) /* Deleting first node */
      {
         ...
         if (hmdma->LastLinkedListNodeAddress == pNode) {
            ...
            if ((uint32_t)hmdma->FirstLinkedListNodeAddress ==
               ...
               /* if last node is looping to first (circular list) one update
                * the last node connection */
                         ...
                         uint32_t Timeout)
{
   ...
   if (CompleteLevel == HAL_MDMA_BUFFER_TRANSFER) {
      ...
      __HAL_MDMA_CLEAR_FLAG(hmdma, (MDMA_FLAG_BRT | MDMA_FLAG_BT |
                                    MDMA_FLAG_BFTC | MDMA_FLAG_CTC));
