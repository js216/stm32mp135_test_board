...
int main(void)
{
   ...
   /* Prescaler 203 -> ~1 MHz from 204 MHz quadspi_ker_ck.  FSIZE = 23
    * (16 MB fictitious), CSHT = 1, no sample shift. */
   if (qspi_init(203U, 23U, 1U, 0U) != HAL_OK) {
      printf("qspi_init failed\r\n");
      Error_Handler();
   }
   printf("qspi: presc=203 fsize=23 csht=1\r\n");
   ...
   while (1) {
      HAL_StatusTypeDef s = qspi_xfer(&id_cmd, QSPI_FMODE_READ, id, 100U);
      iters++;
      if (s != HAL_OK) {
         printf("xfer fail %d at iter %lu\r\n", (int)s, (unsigned long)iters);
         Error_Handler();
      }
      const uint32_t t = HAL_GetTick();
      if (t - last_print >= 1000U) {
         last_print = t;
         printf("JEDEC ID: %02x %02x %02x  (%lu reads)\r\n", id[0], id[1],
                id[2], (unsigned long)iters);
      }
   }
}
...
void PeriphCommonClock_Config(void)
{
   ...
   */
   ...
   /* QSPI kernel clock from ACLK (~204 MHz with this PLL setup). */
