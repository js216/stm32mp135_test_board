...
typedef struct {
   ...
   uint32_t
       WaitForInterrupt; /*!< Specifies whether SDMMC wait for interrupt request
                            ...
                            value of @ref SDMMC_LL_Wait_Interrupt_State  */
...
#define SDMMC_ERROR_BLOCK_LEN_ERR                                              \
   ((uint32_t)0x00000080U) /*!< Transferred block length is not allowed for    \
                              the card or the number of transferred bytes does \
...
#define SDMMC_ERROR_LOCK_UNLOCK_FAILED                                         \
   ((uint32_t)0x00000800U) /*!< Sequence or password error has been detected   \
                              in unlock command or if there was an attempt to  \
...
#define SDMMC_CMD_ALL_SEND_CID                                                 \
   ((uint8_t)2U) /*!< Asks any card connected to the host to send the CID      \
...
#define SDMMC_CMD_SDMMC_SEN_OP_COND                                            \
   ((uint8_t)5U) /*!< Sends host capacity support information (HCS) and asks   \
                    the accessed card to send its operating condition register \
                    (OCR) content in the response on the CMD line.*/
#define SDMMC_CMD_HS_SWITCH                                                    \
   ((uint8_t)6U) /*!< Checks switchable function (mode 0) and switch card      \
                    function (mode 1).                   */
#define SDMMC_CMD_SEL_DESEL_CARD                                               \
   ((uint8_t)7U) /*!< Selects the card by its own relative address and gets    \
                    deselected by any other address    */
#define SDMMC_CMD_HS_SEND_EXT_CSD                                              \
   ((uint8_t)8U) /*!< Sends SD Memory Card interface condition, which includes \
                    host supply voltage information  and asks the card whether \
                    card supports voltage.                      */
#define SDMMC_CMD_SEND_CSD                                                     \
   ((uint8_t)9U) /*!< Addressed card sends its card specific data (CSD) on the \
                    CMD line.                       */
#define SDMMC_CMD_SEND_CID                                                     \
   ((uint8_t)10U) /*!< Addressed card sends its card identification (CID) on   \
                     the CMD line.                      */
...
#define SDMMC_CMD_SET_BLOCKLEN                                                 \
   ((uint8_t)16U) /*!< Sets the block length (in bytes for SDSC) for all       \
                     following block commands (read, write, lock). Default     \
                     block length is fixed to 512 Bytes. Not effective */
/*!< for SDHS and SDXC. */
#define SDMMC_CMD_READ_SINGLE_BLOCK                                            \
   ((uint8_t)17U) /*!< Reads single block of size selected by SET_BLOCKLEN in  \
                     case of SDSC, and a block of fixed 512 bytes in case of   \
                     SDHC and SDXC.                                    */
#define SDMMC_CMD_READ_MULT_BLOCK                                              \
   ((uint8_t)18U) /*!< Continuously transfers data blocks from card to host    \
...
#define SDMMC_CMD_WRITE_SINGLE_BLOCK                                           \
   ((uint8_t)24U) /*!< Writes single block of size selected by SET_BLOCKLEN in \
                     case of SDSC, and a block of fixed 512 bytes in case of   \
                     SDHC and SDXC.                                   */
...
#define SDMMC_CMD_SD_ERASE_GRP_START                                           \
   ((uint8_t)32U) /*!< Sets the address of the first write block to be erased. \
                     (For SD card only).              */
#define SDMMC_CMD_SD_ERASE_GRP_END                                             \
   ((uint8_t)33U) /*!< Sets the address of the last write block of the         \
                     continuous range to be erased.           */
#define SDMMC_CMD_ERASE_GRP_START                                              \
   ((uint8_t)35U) /*!< Sets the address of the first write block to be erased. \
                     Reserved for each command system set by switch function   \
                     command (CMD6).                                  */
#define SDMMC_CMD_ERASE_GRP_END                                                \
   ((uint8_t)36U) /*!< Sets the address of the last write block of the         \
...
#define SDMMC_CMD_LOCK_UNLOCK                                                  \
   ((uint8_t)42U) /*!< Sets/resets the password or lock/unlock the card. The   \
                     size of the data block is set by the SET_BLOCK_LEN        \
                     command. */
#define SDMMC_CMD_APP_CMD                                                      \
   ((uint8_t)55U) /*!< Indicates to the card that the next command is an       \
                     application specific command rather than a standard       \
                     command. */
#define SDMMC_CMD_GEN_CMD                                                      \
   ((uint8_t)56U) /*!< Used either to transfer a data block to the card or to  \
...
#define SDMMC_CMD_APP_SD_SET_BUSWIDTH                                          \
   ((uint8_t)6U) /*!< (ACMD6) Defines the data bus width to be used for data   \
...
#define SDMMC_CMD_SD_APP_SEND_NUM_WRITE_BLOCKS                                 \
   ((uint8_t)22U) /*!< (ACMD22) Sends the number of the written (without       \
                     errors) write blocks. Responds with 32bit+CRC data block. \
                   */
#define SDMMC_CMD_SD_APP_OP_COND                                               \
   ((uint8_t)41U) /*!< (ACMD41) Sends host capacity support information (HCS)  \
                     and asks the accessed card to send its operating          \
                     condition register (OCR) content in the response on the   \
                     CMD line. */
#define SDMMC_CMD_SD_APP_SET_CLR_CARD_DETECT                                   \
   ((uint8_t)42U) /*!< (ACMD42) Connect/Disconnect the 50 KOhm pull-up         \
                     resistor on CD/DAT3 (pin 1) of the card  */
...
#define SDMMC_STATIC_FLAGS                                                     \
   ((uint32_t)(SDMMC_FLAG_CCRCFAIL | SDMMC_FLAG_DCRCFAIL |                     \
               ...
               SDMMC_FLAG_SDIOIT | SDMMC_FLAG_ACKFAIL |                        \
