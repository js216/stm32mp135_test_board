...
typedef union {
   ...
   struct {
      ...
      uint32_t RADIS : 1;  /*!< \brief bit:    12  L1 Data Cache read-allocate
                              mode disable */
   ...
   struct {
      ...
      uint32_t EXCL
          : 1;          /*!< \brief bit:     7  Exclusive L1/L2 cache control */
      uint32_t AOW : 1; /*!< \brief bit:     8  Enable allocation in one cache
                           way only */
      uint32_t PARITY : 1; /*!< \brief bit:     9  Support for parity checking,
                              if implemented */
...
typedef struct {
   ...
   __IOM uint32_t IGROUPR[32]; /*!< \brief  Offset: 0x080 (R/W) Interrupt Group
                                  Registers */
...
#define GICInterface                                                           \
   ((GICInterface_Type *)GIC_INTERFACE_BASE) /*!< \brief GIC Interface         \
                                                register set access pointer */
#endif
...
typedef union {
   struct {
      ...
      uint32_t IMASK
          : 1; /*!< \brief bit: 1      Timer output signal mask bit. */
   ...
   *descriptor_l1 &= SECTION_TEXCB_MASK;
   ...
   if (STRONGLY_ORDERED == mem) {
      ...
      *descriptor_l1 |= 1 << SECTION_TEX2_SHIFT;
      switch (inner) {
         case NON_CACHEABLE: break;
         case WB_WA: *descriptor_l1 |= (1 << SECTION_B_SHIFT); break;
         case WT: *descriptor_l1 |= 1 << SECTION_C_SHIFT; break;
         case WB_NO_WA:
      ...
      switch (outer) {
         case NON_CACHEABLE: break;
         case WB_WA: *descriptor_l1 |= (1 << SECTION_TEX0_SHIFT); break;
         case WT: *descriptor_l1 |= 1 << SECTION_TEX1_SHIFT; break;
         case WB_NO_WA:
            *descriptor_l1 |=
   ...
   if (page == PAGE_64k) {
      ...
      if (STRONGLY_ORDERED == mem) {
         ...
         *descriptor_l2 |= 1 << PAGE_4K_TEX2_SHIFT;
         switch (inner) {
            case NON_CACHEABLE: break;
            case WB_WA: *descriptor_l2 |= (1 << PAGE_4K_B_SHIFT); break;
            case WT: *descriptor_l2 |= 1 << PAGE_4K_C_SHIFT; break;
            case WB_NO_WA:
               *descriptor_l2 |=
         ...
         switch (outer) {
            case NON_CACHEABLE: break;
            case WB_WA: *descriptor_l2 |= (1 << PAGE_4K_TEX0_SHIFT); break;
            case WT: *descriptor_l2 |= 1 << PAGE_4K_TEX1_SHIFT; break;
            case WB_NO_WA:
               *descriptor_l2 |=
                                          ...
                                          mmu_region_attributes_Type reg)
{
   ...
   *descriptor2 = 0;
   ...
   switch (reg.rg_t) {
      case PAGE_4k:
         MMU_MemoryPage(descriptor2, reg.mem_t, reg.outer_norm_t,
                        reg.inner_norm_t, PAGE_4k);
